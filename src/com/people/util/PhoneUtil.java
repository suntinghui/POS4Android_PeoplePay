...
public class PhoneUtil {
	...
	public static void sendSMS(String phoneNum, String smsCnt) {
		try{
			...
			BaseActivity.getTopActivity().runOnUiThread(new Runnable(){
				@Override
				public void run() {
					Toast.makeText(BaseActivity.getTopActivity(), "?????????ä¿¡å¤±è´?", Toast.LENGTH_SHORT).show();
				}
	...
	public static void sendMMS(String phoneNum){
	    ...
	    sendIntent.putExtra("subject", "äº¤æ??ç­¾å??");
	    sendIntent.putExtra("sms_body", "å·²æ?????å®????äº¤æ??ï¼????ä¸?ä¿¡æ??ç­¾å??ï¼?è¯·æ³¨?????¥æ?¶ä??å­?");
